...
@Component
public class TierOneContractValidation implements IContractValidation {
    @Autowired
    ...
    public Boolean isOrganizationAllowed() {
        ...
        return activeCount < EValidation.ACTIVE_ORGS_ALLOWED.getNums();
    }

    @Override
    public Boolean isMasterAgreementAllowed(List<Contract> masterContracts) {
        return masterContracts.size() < EValidation.ACTIVE_MASTER_CONTRACTS_ALLOWED.getNums();
    }
    ...
    enum EValidation {
        ACTIVE_ORGS_ALLOWED(1),
        ACTIVE_MASTER_CONTRACTS_ALLOWED(1);
        int nums;
