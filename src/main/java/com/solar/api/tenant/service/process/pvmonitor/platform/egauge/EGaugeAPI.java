...
@Service
@Deprecated
public class EGaugeAPI implements MonitorAPI {
    ...
    public MonitorAPIAuthResponse getAuthData(Object... params) throws NoSuchAlgorithmException {
        Map<String, APIConstants> constants = PrerequisitesFactory.getAPIPrerequisites(Constants.MONITOR_PLATFORM.EGAUGE).getConstants();
    ...
    public MonitorAPIResponse getCurrentData(CustomerSubscription cs, Object... params) throws NoSuchAlgorithmException {
        ...
        ResponseEntity<MonitorAPIResponse> gpResponse = getAPIData(null, "GET_POWER", body, constants, "?currentTime=" + time, MonitorAPIResponse.class, true);
        ResponseEntity<MonitorAPIResponse> cdResponse = getAPIData(null, "GET_CURRENT_DATA", body, constants, "?currentTime=" + time, MonitorAPIResponse.class, true);
        ResponseEntity<List<String>> asResponse = getAPIData(null, "ALL_SITES", body, constants, "?userId=" + body.getUserId(), List.class, true);
        ResponseEntity<MonitorAPIResponse> spResponse = (ResponseEntity<MonitorAPIResponse>) getAPIData(null, "GET_SITE_TOTAL_POWER", body, constants, "?siteId=" + asResponse.getBody().get(0) + "&time=" + time.split(" ")[0], MonitorAPIResponse.class, true);
    ...
    public MonitorAPIResponse getCurrentData(ExtDataStageDefinition ext, Object... params) throws NoSuchAlgorithmException {
        ...
        MonitorAPIAuthBody body = getAuthBody(filteredRateCodes);
        String time = (String) params[0] == null ? format.format(new Date()) : (String) params[0];
        ResponseEntity<MonitorAPIResponse> gpResponse = getAPIData(ext, "GET_POWER", body, constants, "?currentTime=" + time, MonitorAPIResponse.class, true);
        ResponseEntity<MonitorAPIResponse> cdResponse = getAPIData(ext, "GET_CURRENT_DATA", body, constants, "?currentTime=" + time, MonitorAPIResponse.class, true);
        ResponseEntity<List<String>> asResponse = getAPIData(ext, "ALL_SITES", body, constants, "?userId=" + body.getUserId(), List.class, true);
        ResponseEntity<MonitorAPIResponse> spResponse = (ResponseEntity<MonitorAPIResponse>) getAPIData(ext, "GET_SITE_TOTAL_POWER", body, constants, "?siteId=" + asResponse.getBody().get(0) + "&time=" + time.split(" ")[0], MonitorAPIResponse.class, true);
    ...
    private ResponseEntity getAPIData(ExtDataStageDefinition ext, String apiName, MonitorAPIAuthBody body,
        ...
        try {
            ...
            if (ext != null && !status.startsWith("20")) {
                monitorPlatformUtilityService.outages(ext);
                LOGGER.error("Error triggering outages for EGauge. SubscriptionId: {}, Status code: {}", ext.getSubsId(), status);
            }
        } catch (Exception e) {
            LOGGER.warn(e.getMessage());
        }
    ...
    private MonitorAPIAuthBody getAuthBody(Map<String,String> filteredRateCodes ) throws NoSuchAlgorithmException {
        MonitorAPIAuthResponse authResponse = getAuthData(filteredRateCodes.get(Constants.RATE_CODES.LGNM), filteredRateCodes.get(Constants.RATE_CODES.PCD));
    ...
    public List<MonitorReading> getMonitorReadingDataForUserComparison(Long userId, MonitorAPIAuthBody body, List<CustomerSubscription> customerSubscriptionList) throws ParseException {
      List<MonitorReading>  daysData = new ArrayList<>();
        if(customerSubscriptionList.size()>0) {
            List<Date> labelDatesEgauge = getDateTimes(body.getTime());
            daysData  = readingRepository.findByUserIdAndTimeIn(userId, labelDatesEgauge, customerSubscriptionList);
        }
//        return daysData;
        return Collections.EMPTY_LIST;
    }
