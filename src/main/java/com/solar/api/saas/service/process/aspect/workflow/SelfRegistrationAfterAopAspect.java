...
@Aspect
@Configuration
public class SelfRegistrationAfterAopAspect {
    ...
    @AfterReturning(value = "execution(* com.solar.api.tenant.service.UserServiceImpl.userSelfRegistration(..))", returning = "result")
    public void afterReturning(JoinPoint joinPoint, Object result) {
        ...
        if (result instanceof User) {
            ...
            try {
                ...
                String tenantName = company.getCompanyName();//companyPreferenceService.getCurrentCompanyPreference(user.getCompKey()).getCompanyName();
                String saltStr = externalLinkService.verifyUser(userEmail, user.getCompKey(), SUBJECT);
//                String url =  feHostC + "/#/verified" + "?ID=" + saltStr + "&TID=" + user.getCompKey();
//                "https://sistage.azurewebsites.net/#/verified"
                String url = feHostA + "/#/verified" + "?ID=" + saltStr + "&TID=" + user.getCompKey();
                ...
                hookValidator.hookFinder(null, "self_registration", user.getAcctId(), null, placeholderValues, null, subjectParams);
    ...
    @After(value = "execution(* com.solar.api.tenant.service.UserServiceImpl.userSelfRegistration(..))")
