...
@Configuration
public class WeatherDataStoringBatchConfig {
    ...
    public Job batchJob(JobBuilderFactory jobBuilderFactory,
        ...
        Step step = stepBuilderFactory.get("SAVING WEATHER DATA")
                .tasklet(saveWeatherData())

                .build();
        return jobBuilderFactory.get(AppConstants.WEATHER_API_FOR_SEVEN_DAYS)
                .incrementer(new RunIdIncrementer())
                .start(step)
                .build();
    }

    private Tasklet saveWeatherData() {
        return (contribution, chunkContext) -> {
            chunkContext.getStepContext().getStepExecutionContext();
            try {
                weatherService.getSevenDayDailyWeatherData((Long) chunkContext.getStepContext().getJobParameters().get("compKey"),null);
            } catch (Exception ex) {
